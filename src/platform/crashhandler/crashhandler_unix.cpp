...
namespace {
   ...
   void signalHandler(int signal_number, siginfo_t* info, void* unused_context) {
      ...
      using namespace g3::internal;
      {
         ...
         const auto fatal_reason = exitReasonName(g3::LEVELS::critical, signal_number);
         ...
         LogCapture trigger(g3::LEVELS::critical, static_cast<g3::SignalType>(signal_number), dump.c_str());
   ...
   namespace internal {
      ...
      std::string exitReasonName(const LEVELS& level, g3::SignalType fatal_id) {
         ...
         switch (signal_number) {
            ...
            default:
               std::ostringstream oss;
               oss << "UNKNOWN SIGNAL(" << signal_number << ") for " << spdlog::level::to_str(level);
               return oss.str();
         }
      }
      ...
      void exitWithDefaultSignalHandler(const LEVELS& level, g3::SignalType fatal_signal_id) {
         ...
         std::cerr << "\n\n" << __FUNCTION__ << ":" << __LINE__ << ". Exiting due to " << spdlog::level::to_str(level) << ", " << signal_number << "   \n\n" << std::flush;
