...
public:
    RouteGuideClient(std::shared_ptr<Channel> channel, const std::string& db)
        : m_stub(RouteGuide::NewStub(channel))
    {
...
//         std::unique_ptr<ClientReader<Feature>> reader;
        auto reader = m_stub->ListFeatures(&context, rect);
    ...
    void RecordRoute()
    {
        ...
        std::uniform_int_distribution<int> feature_distribution(0, m_feature_list.size() - 1);
        std::uniform_int_distribution<int> delay_distribution(500, 1500);

//         std::unique_ptr<ClientWriter<Point>> writer;
        auto writer = m_stub->RecordRoute(&context, &stats);
        for (int i = 0; i < kPoints; i++) {
            const Feature& f = m_feature_list[feature_distribution(generator)];
...
private:
    ...
    {
        ClientContext context;
        Status status = m_stub->GetFeature(&context, point, feature);
        if (!status.ok()) {
