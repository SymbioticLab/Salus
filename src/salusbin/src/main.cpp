...
static auto kVersion = "Salus: Fine-Grained GPU Sharing for DNN version " PROJECT_VERSION ""s;
...
{
    return value_or<std::optional<T>, T>(v, std::nullopt);
}
...
{
    auto disableAdmissionControl = value_or(args[flags::disableAdmissionControl], false);
    SessionResourceTracker::instance().setDisabled(disableAdmissionControl);

    auto disableFairness = value_or(args[flags::disableFairness], false);
    auto maxQueueHeadWaiting = value_or(args[flags::maxHolWaiting], 50u);
    auto disableWorkConservative = value_or(args[flags::disableWorkConservative], false);
    auto sched = value_or(args[flags::scheduler], "fair"s);
