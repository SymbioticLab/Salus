'''
Created Date: Friday, August 23rd 2019, 9:25:48 pm
Author: Yuchen Zhong
...
training_job_name = "alexnet_25.salus.250iter.0.output"
inference_job_names = ["resnet50eval_1.salus.500iter.1.output",
                       "resnet50eval_1.salus.500iter.2.output" 
                       "resnet50eval_1.salus.500iter.3.output" 
                       ]
log_path = "scripts/templogs/zyc3"
policy_list = [
...
if __name__=="__main__":   
    ...
    for policy in policy_list:
        ...
        plt.yticks(np.arange(min_v, max_v, 20))
        line, = plt.plot(speeds, linewidth=5.0)
        lines.append(line)
    plt.legend(lines, policy_list, prop={'size':30})
    plt.title("exp3 - " + training_job_name)
    plt.xlabel("iter")
    plt.ylabel("images/sec")
    # plt.show()
    os.system("mkdir -p scripts/imgs/exp3")
    plt.savefig('scripts/imgs/exp3/' + training_job_name + ".png", dpi=fig.dpi)
